...
for (const path of runtimePaths) {
  if (!pathIsFile(path)) failures.push(`Runtime audio does not resolve to a file under public/: ${path}`);
...
for (const path of ledgerStatuses.keys()) {
  if (!pathIsFile(path)) failures.push(`Audio provenance row does not resolve to a file: ${path}`);
}
