...
15. Verify that the seeded Guild rows are exactly `history`, `nature`, `walk`, and `rebellion`, then test the public Guild leaderboard before enabling the feature for players.
16. Note that Guild score events are idempotent by authenticated user and client run ID, but the existing GPS check remains browser-side in this MVP and is not a server-side anti-cheat proof.
